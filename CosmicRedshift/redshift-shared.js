...
// ==========================================
const redshiftQuizData = {
    's1Q1': { dbNum: 1, correctLetter: 'C', correctIndex: 2 },  // space itself expands in transit
    's1Q2': { dbNum: 2, correctLetter: 'C', correctIndex: 2 },  // 450 nm at z=1 -> 900 nm
    's2Q1': { dbNum: 3, correctLetter: 'B', correctIndex: 1 },  // crossover ~6.3 Gly, z ~ 0.67
    's2Q2': { dbNum: 4, correctLetter: 'D', correctIndex: 3 },  // arrives mid-IR, just over 5 microns
    's3Q1': { dbNum: 5, correctLetter: 'A', correctIndex: 0 },  // blue -> red supergiant -> SN -> BH
    's3Q2': { dbNum: 6, correctLetter: 'B', correctIndex: 1 }   // light from sources that no longer exist
};
...
function handleQuizAnswer(questionId, selectedOptionElement, selectedLetter) {
    ...
    const qData = redshiftQuizData[questionId];
    if (!qData) return;
...
function restoreAnswers() {
    ...
    Object.keys(studentData.answers).forEach(dbNumStr => {
        ...
        for (const [key, value] of Object.entries(redshiftQuizData)) {
