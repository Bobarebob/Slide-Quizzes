...
function handleQuizAnswer(questionNumber, optionIndex) {
    // Once the quiz is submitted -- by the student on the outro page, or
    // automatically when the timer runs out -- the content stays readable but
    // no further answers are accepted.
    const now = Date.now();
...
function applyAnswerFeedback(questionNumber, selectedOption, correctOption) {
    ...
    options.forEach(opt => {
        ...
        opt.classList.add('answered-styled');
    });
...
// Grey out and disable every remaining option on the page. Used after the quiz
// is submitted: the student may still page back through the content, but any
// question left unanswered stays unanswered.
function lockAllOptions() {
    document.querySelectorAll('.quiz-option').forEach(opt => {
        opt.style.pointerEvents = 'none';
        opt.style.cursor = 'not-allowed';
        if (!opt.classList.contains('answered-styled')) opt.style.opacity = '0.55';
...
function restoreAnswers() {
    ...
    // If the quiz is already over, lock whatever is still open on this page.
    const now = Date.now();
    if (sessionStorage.getItem('quizCompleted') === 'true' || (timerEndTime > 0 && now > timerEndTime)) {
        sessionStorage.setItem('quizCompleted', 'true');
        lockAllOptions();
    ...
    timerInterval = setInterval(() => {
        ...
        if (warningMsg) {
            if (timeRemainingSeconds <= 0) {
                // Auto-submit. Answering closes here; reading does not.
                sessionStorage.setItem('quizCompleted', 'true');
                if (!sessionStorage.getItem('completionTime')) {
                    sessionStorage.setItem('completionTime', timerEndTime.toString());
                }
                lockAllOptions();
                ...
                setTimeout(() => {
                    ...
                    } else if (typeof showSubmittedState === 'function') {
                        showSubmittedState();
                    }
                }, 100);
...
function submitQuiz() {
    sessionStorage.setItem('quizCompleted', 'true');
    if (!sessionStorage.getItem('completionTime')) {
        sessionStorage.setItem('completionTime', Date.now().toString());
    }
    if (db) {
