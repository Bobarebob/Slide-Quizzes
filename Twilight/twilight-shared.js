...
// 6 questions total: 2 on slide1, 2 on slide2, 2 on slide3
// ==========================================
const twilightQuizData = {
    's1Q1': { dbNum: 1, correctLetter: 'B', correctIndex: 1 },  // longer: the path leans further from vertical
    's1Q2': { dbNum: 2, correctLetter: 'C', correctIndex: 2 },  // at 20N the lean is about 20 degrees
    's2Q1': { dbNum: 3, correctLetter: 'B', correctIndex: 1 },  // horizon plus bright stars, both at once
    's2Q2': { dbNum: 4, correctLetter: 'B', correctIndex: 1 },  // straight down, equal 6 degree bands
    's3Q1': { dbNum: 5, correctLetter: 'C', correctIndex: 2 },  // never reaches 18 below, stage never starts
    's3Q2': { dbNum: 6, correctLetter: 'B', correctIndex: 1 }   // midnight Sun: no sunset, so no twilight
};
...
function handleQuizAnswer(questionId, selectedOptionElement, selectedLetter) {
    ...
    const qData = twilightQuizData[questionId];
    if (!qData) return;
...
function restoreAnswers() {
    ...
    Object.keys(studentData.answers).forEach(dbNumStr => {
        ...
        for (const [key, value] of Object.entries(twilightQuizData)) {
